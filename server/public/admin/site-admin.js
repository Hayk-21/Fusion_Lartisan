...
(function () {
  ...
  function renderHoursGrid(hours) {
    ...
    g.innerHTML = `<div class="h"></div><div class="h">${t('site.opens')}</div><div class="h">${t('site.closes')}</div><div class="h">${t('site.closedDay')}</div>` +
      DAYS.map(d => { const h = (hours || {})[d] || { open: '08:00', close: '20:00', closed: false }; return `<div><b>${dayName(d)}</b></div><input type="time" data-h="${d}" data-k="open" value="${h.open}"><input type="time" data-h="${d}" data-k="close" value="${h.close}"><input type="checkbox" data-h="${d}" data-k="closed" ${h.closed ? 'checked' : ''}>`; }).join('');
  }
  function readHours() {
    const out = {};
  ...
  function renderState(st) {
    ...
    if (st.test_mode) { el.innerHTML = `🧪 ${t('site.stTest')} <span class="muted">(${esc(st.now_local || '')})</span>`; return; }
  ...
  async function loadSite() {
    const [s, st] = await Promise.all([api('/admin/settings'), api('/admin/site/state')]);
    $('#wTestMode').checked = !!s.test_mode; $('#wHoursGoogle').checked = s.hours_from_google !== false; $('#wAboutGoogle').checked = s.about_from_google !== false;
    $('#wFeaturedCount').value = s.featured_count || 4;
  ...
  function syncHoursUi(hasKey) {
    const auto = $('#wHoursGoogle').checked;
    $$('#hoursGrid input').forEach(i => i.disabled = auto);
    $('#hoursNote').textContent = auto ? (hasKey ? t('site.hoursAuto') : t('site.hoursNoKey')) : t('site.hoursManual');
  }
  $('#wHoursGoogle').addEventListener('change', () => syncHoursUi(true));
  ...
  $('#siteSave').addEventListener('click', async () => {
    ...
    const patch = {
      ...
      test_mode: $('#wTestMode').checked, hours_from_google: $('#wHoursGoogle').checked, about_from_google: $('#wAboutGoogle').checked, featured_count: Number($('#wFeaturedCount').value) || 4,
    };
  ...
  $('#reviewsRefresh').addEventListener('click', async () => {
    ...
    try { const r = await api('/admin/site/reviews/refresh'); $('#reviewsInfo').textContent = r && r.rating ? t('site.reviewsOk', { r: r.rating, n: r.count }) : (r?.error || t('site.reviewsNone')); toast(t('common.saved'), 'ok'); loadSite(); }
