...
(function () {
  ...
  const STR = {
    fr: { tagline: 'Crêperie & Café', 'nav.menu': 'Menu', 'nav.reviews': 'Avis', 'nav.contact': 'Nous trouver', 'nav.order': 'Commander', 'hero.order': 'Commander en ligne', 'hero.menu': 'Voir le menu',
      ...
      'dishes.title': 'Quelques-unes de nos créations', 'dishes.sub': 'Fait maison chaque jour, avec des ingrédients simples, frais et bio.', 'dishes.all': 'Tout le menu →', from: 'à partir de',
      'about.title': 'Notre histoire', 'about.motto': "L'art du fait maison.", 'v.fresh': 'Produits frais et bio', 'v.home': 'Fait maison chaque jour', 'v.coffee': 'Café de spécialité', 'v.breton': 'Inspiration bretonne', 'v.gf': 'Galettes de sarrasin sans gluten', 'v.welcome': 'Accueil chaleureux à Montréal',
      'reviews.title': 'Ce que disent nos clients', 'reviews.link': 'Voir les avis sur Google →', 'reviews.count': '{n} avis Google', 'reviews.none': 'Les avis Google apparaîtront ici.',
      'loc.title': 'Nous trouver', 'loc.hours': "Heures d'ouverture", closed: 'Fermé', 'foot.tag': 'Crêperie artisanale bretonne · Café de spécialité · Sur place & à emporter', 'foot.thanks': "Merci de soutenir l'artisanat local !",
      open: 'Ouvert', 'open.until': 'Ouvert · ferme à {t}', 'closed.now': 'Fermé', 'closed.today': "Fermé aujourd'hui", 'closed.temp': 'Fermé exceptionnellement', 'opens.today': 'ouvre à {t}', 'opens.tomorrow': 'ouvre demain à {t}', 'opens.day': 'ouvre {d} à {t}', 'hours.today': "Aujourd'hui : {o} – {c}",
    ...
    en: { tagline: 'Crêperie & Café', 'nav.menu': 'Menu', 'nav.reviews': 'Reviews', 'nav.contact': 'Find us', 'nav.order': 'Order', 'hero.order': 'Order online', 'hero.menu': 'See the menu',
      ...
      'dishes.title': 'A few of our creations', 'dishes.sub': 'Made in-house every day with simple, fresh, organic ingredients.', 'dishes.all': 'Full menu →', from: 'from',
      'about.title': 'Our story', 'about.motto': 'The art of made in-house.', 'v.fresh': 'Fresh, organic products', 'v.home': 'Made in-house daily', 'v.coffee': 'Specialty coffee', 'v.breton': 'Breton inspiration', 'v.gf': 'Gluten-free buckwheat galettes', 'v.welcome': 'A warm welcome in Montréal',
      'reviews.title': 'What our customers say', 'reviews.link': 'See reviews on Google →', 'reviews.count': '{n} Google reviews', 'reviews.none': 'Google reviews will appear here.',
      'loc.title': 'Find us', 'loc.hours': 'Opening hours', closed: 'Closed', 'foot.tag': 'Artisan Breton crêperie · Specialty coffee · Dine in & take out', 'foot.thanks': 'Thank you for supporting local artisans!',
      open: 'Open', 'open.until': 'Open · closes at {t}', 'closed.now': 'Closed', 'closed.today': 'Closed today', 'closed.temp': 'Exceptionally closed', 'opens.today': 'opens at {t}', 'opens.tomorrow': 'opens tomorrow at {t}', 'opens.day': 'opens {d} at {t}', 'hours.today': 'Today: {o} – {c}',
  ...
  function render() {
    ...
    let text, hint = '';
    if (st.open) { text = t('open.until', { t: fmtT(st.today.close) }); }
    else {
    ...
    $('#openText').textContent = text; $('#hoursToday').textContent = hint ? '· ' + hint : (st.today && !st.today.closed ? '· ' + t('hours.today', { o: fmtT(st.today.open), c: fmtT(st.today.close) }) : '');
    ...
    if (r && r.rating) {
      ...
      $('#reviewList').innerHTML = (r.reviews || []).filter(x => x.text).slice(0, 6).map(x => `<div class="review"><div class="who">${x.photo ? `<img src="${esc(x.photo)}" alt="" referrerpolicy="no-referrer">` : '<img alt="">'}<div><b>${esc(x.author)}</b><small>${esc(x.when)}</small></div></div><div class="stars">${'★'.repeat(x.rating)}${'☆'.repeat(5 - x.rating)}</div><p>${esc(x.text)}</p></div>`).join('');
